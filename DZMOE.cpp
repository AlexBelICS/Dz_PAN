...
const double TU154_NOMINAL_THRUST = 3 * 103000.0; // Суммарная тяга, Н
const double TU154_CY0 = 0.25;               // Коэффициент подъемной силы при нулевом угле атаки
const double MAX_THRUST_PERCENT = 1;     // Реалистичный процент тяги
...
public:
    ...
    void plotTrajectory() const {
        ...
        fprintf(gp, "set multiplot layout 2,2 title 'Параметры полета Tу-154'\n");
        ...
        fprintf(gp, "e\n");
        // Высота от скорости
        fprintf(gp, "set title 'Высота от скорости'\n");
        fprintf(gp, "set xlabel 'Скорость, км/ч'\n");
        fprintf(gp, "set ylabel 'Высота, м'\n");
        fprintf(gp, "set grid\n");
        fprintf(gp, "set yrange[0:5000]\n");
        fprintf(gp, "plot '-' with lines lw 2 lc 'orange' title 'Высота'\n");
        for (const auto& point : points) {
        	fprintf(gp, "%f %f\n", point.V*3.6, point.y);
        }
        fprintf(gp, "e\n");
...
public:
    ...
    double calculate_alpha(double current_altitude, double current_velocity) {
        ...
        double alpha = (TU154_MASS * GRAVITY - q * TU154_WING_AREA * TU154_CY0) / (q * TU154_WING_AREA * grad_Cy + current_P);
        return alpha;
...
double getLiftCoefficient(double alpha) const{
    ...
    double computeLiftForce(double current_velocity, double current_altitude, double alpha) const {
        ...
        double L = 0.5 * rho * current_velocity * current_velocity * TU154_WING_AREA * Cy;
        return L;
    }
    ...
    double computeDragForce(double current_velocity, double current_altitude, double alpha) const {
        ...
        double X = 0.5 * rho * current_velocity * current_velocity * TU154_WING_AREA * Cx;
        return X;
    }
    ...
    double total_thrust(double current_altitude){
        ...
        return TU154_NOMINAL_THRUST*MAX_THRUST_PERCENT * pow(current_p/p_0, 0.7); // коррекция тяги по плотности и давлению
    }
};
...
public:
    ...
    double calculate_razgon(double altitude, double initial_velocity, double final_velocity, Aircraft& ac){
        ...
        double a_x = ((current_P * cos(alpha_rad)) - q * Cx * TU154_WING_AREA) / TU154_MASS;
    ...
    double calculate_podiem(double initial_alt, double final_alt, double velocity, Aircraft& ac){
        ...
        double sin_tetha = std::min((current_P-X) / (TU154_MASS* GRAVITY), MAX_CLIMB_ANGLE/DEG_TO_RAD);
...
int main() {
    ...
    try {
        ...
        trajectory.saveToCSV("tu154_trajectory_realistic.csv");
